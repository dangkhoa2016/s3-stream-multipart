...
required_vars = %w[S3_BUCKET S3_ACCESS_KEY_ID S3_SECRET_ACCESS_KEY LOCAL_FILE_PATH S3_OBJECT_KEY]
...
log_file = ENV['S3_LOG_FILE'] || File.join(__dir__, 'upload_with_s3_client.log')
...
# ============================================================
# Setup 2 loggers:
#   - file_logger: DEBUG level, saves ALL details to log file
#   - console_logger: INFO level, prints important messages to screen
# ============================================================
...
# Uses a custom class instead of delegator to avoid gem dependency.
#
# S3Client.setup_logger always calls logger.formatter= and logger.level= when
# receiving an external logger. If forwarded to inner loggers, it would override
# each logger's formatter/level (file: DEBUG + full timestamp,
# console: INFO + short timestamp). -> Make it no-op here, preserve caller config.
class MultiLogger
  ...
  # level= / formatter= only stored in MultiLogger (to keep setup_logger happy),
  # does NOT forward to inner loggers.
...
begin
  puts C.bold("Initializing S3Client...")

  # combined_logger receives both DEBUG and INFO — client logs details through it
  client = S3Client.new(
  ...
  # --- :upload_start ---
  S3Client.on(:upload_start) do |lp, key, size, total_parts, part_size, resumed|
    stats[:t0] = Time.now
    ...
    puts "║ concurrency: #{client.max_concurrency}"
    puts "╚" + "═" * 40
    client.log_info("[CALLBACK] upload_start: key=#{key} size=#{size} total_parts=#{total_parts} resumed=#{resumed}")
  end
  ...
  S3Client.on(:upload_resume) do |state|
    ...
    in_progress = (state[:in_progress_parts] || {}).keys.sort
    puts C.bold("📂 Resume from state:")
    puts "   session:      #{state[:upload_session_id]}"
    puts "   upload_id:    #{state[:upload_id][0, 24]}..."
    puts "   progress:     #{done}/#{total_parts} parts (#{hr_size(done * state[:part_size])}/#{hr_size(state[:total_size])})"
    puts "   pending:      #{pending} parts"
    puts "   in_progress:  #{in_progress.inspect}  (from previous crash)"
    puts "   resume_count: #{state[:resume_count]}"
    puts "   file_md5:     #{state[:file_md5] || 'N/A'}"
    puts "   started_at:   #{state[:started_at]}"
    puts "   last_updated: #{state[:last_updated_at]}"
    client.log_info("[CALLBACK] upload_resume: session=#{state[:upload_session_id]} done=#{done}/#{total_parts} pending=#{pending} in_progress=#{in_progress.inspect}")
  end

  # --- :thread_start / :thread_finish ---
  S3Client.on(:thread_start) do |tid, oid|
    client.log_debug("[CALLBACK] thread_start: #{tid} object_id=#{oid}")
  end

  S3Client.on(:thread_finish) do |tid, oid, count|
  ...
  S3Client.on(:part_complete) do |pn, total, tid, etag, bytes, ms, speed|
    ...
    client.log_info("[CALLBACK] part_complete: #{tid} part=#{pn}/#{total} bytes=#{bytes} " \
                    "time=#{ms.round(1)}ms speed=#{speed} MB/s etag=#{etag[0, 20].inspect} " \
  ...
  # --- :state_save ---
  S3Client.on(:state_save) do |snapshot, completed, total, tid|
    in_prog = (snapshot[:in_progress_parts] || {}).keys.sort
  ...
  S3Client.on(:state_load) do |state, path|
    ...
    puts "   upload_id: #{state[:upload_id][0, 24]}..."
    puts "   session:   #{state[:upload_session_id] || 'N/A'}"
    puts "   parts:     #{state[:parts].size} / #{(state[:total_size].to_f / state[:part_size]).ceil}"
    client.log_info("[CALLBACK] state_load: path=#{path} upload_id=#{state[:upload_id]} parts=#{state[:parts].size}")
  end

  # --- :state_mismatch ---
  S3Client.on(:state_mismatch) do |old_state, new_key, new_size|
    puts C.r("⚠ STATE MISMATCH — aborting old upload")
    puts "   Old: key=#{old_state[:key].inspect} size=#{old_state[:total_size]} session=#{old_state[:upload_session_id]}"
    puts "   New: key=#{new_key.inspect} size=#{new_size}"
    client.log_warn("[CALLBACK] state_mismatch: old_upload_id=#{old_state[:upload_id]} " \
                    "old_key=#{old_state[:key].inspect} new_key=#{new_key.inspect}")
  end
  ...
  S3Client.on(:upload_complete) do |result, elapsed, throughput|
    ...
    client.log_info("[CALLBACK] upload_complete: key=#{result[:key]} etag=#{result[:etag]} " \
                    "parts=#{result[:parts].size} elapsed=#{format('%.3f', elapsed_total)}s " \
  ...
  S3Client.on(:upload_failed) do |err, state_path|
    ...
    puts "║ state:  #{state_path ? "preserved at #{state_path}" : 'not preserved (aborted)'}"
  ...
  # --- :log (custom log từ worker threads) ---
  # Event này fire ngay khi thread_log_* được gọi — dùng để bắt log custom real-time
  S3Client.on(:log) do |level, msg, tid, ts|
    client.log_debug("[CALLBACK:log] level=#{level} thread=#{tid} msg=#{msg}")
  end

  puts C.g("✓ #{S3Client.event_callbacks.values.map(&:size).sum} event callbacks registered")
  puts ""

  # ==========================================================
  # Pre-upload: inspect state file if exists (useful for resume)
  # ==========================================================
  state_file_path = File.join(__dir__, 'upload-state.json')
  if File.exist?(state_file_path)
    begin
      saved = S3Client::UploadState.from_file(state_file_path)
      ...
      if saved.file_md5
        ...
        if current_md5
          if current_md5 == saved.file_md5
            ...
            puts "     Client will abort old upload and start fresh."
          end
        end
      end
      puts ""
  ...
  progress_bytes = proc { |written, total|
    # Callback này fire từ main progress tracking — chỉ log ở DEBUG để tránh spam
  ...
  # Save session_id to display in upload_complete callback
  ...
  # Clear all callbacks after script finishes (to avoid duplicates on next run)
