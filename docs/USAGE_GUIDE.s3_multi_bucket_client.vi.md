...
- [Logging & Quan sát](#logging--quan-sát)
  ...
  - [Event Callbacks — 17 sự kiện vòng đời](#event-callbacks--17-sự-kiện-vòng-đời)
...
state.file_md5                # MD5 của file (nil trừ khi set bên ngoài)
...
### Event Callbacks — 17 sự kiện vòng đời
...
| Sự kiện | Tham số | Khi nào kích hoạt |
|---------|---------|-------------------|
| `:download_start` | `(key, bucket, total_size)` | Download bắt đầu |
| `:download_complete` | `(result, elapsed, throughput)` | Download thành công |
| `:download_failed` | `(error)` | Download thất bại |
...
| `tests/s3_multi_bucket_client/test_client.rb` | 22 | Khởi tạo client, validation, utilities, constants, errors, XML, thread safety |
...
| `upload_directory` | Upload tất cả file trong thư mục |
| `download_file` | Streaming download (hỗ trợ `range:`) |
| `download_stream` | Streaming dạng block |
...
| `S3MultiBucketClient::S3Error` | Lỗi S3 có cấu trúc |
| `S3MultiBucketClient::UploadError` | Lỗi upload cụ thể |
