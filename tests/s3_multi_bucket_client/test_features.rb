...
class S3MultiBucketFeaturesTest < Minitest::Test
  ...
  # --- resume_upload: end-to-end success ---

  def test_resume_upload_from_partial_state
    src_path, src_md5 = create_temp_binary_file(12 * 1024 * 1024)
    dir = suite_tmp_dir("mbc_resume_e2e")
    state_file = File.join(dir, "resume_state.json")
    part_size = 5 * 1024 * 1024
    key = "/resume_e2e.bin"

    uid = @client.create_multipart_upload(bucket: BUCKET, key: key)

    part1_data = File.binread(src_path, part_size, 0)
    part2_data = File.binread(src_path, part_size, part_size)
    etag1 = @client.upload_part(
      bucket: BUCKET, key: key, upload_id: uid, part_number: 1, body: part1_data
    )
    etag2 = @client.upload_part(
      bucket: BUCKET, key: key, upload_id: uid, part_number: 2, body: part2_data
    )
    ...
    state = {
      ...
      completed: false,
      upload_session_id: "mbc_resume_e2e_001",
      started_at: Time.now.utc.iso8601
    }
    FileUtils.mkdir_p(dir)
    File.write(state_file, JSON.generate(state))

    r = @client.resume_upload(bucket: BUCKET, state_file: state_file, local_path: src_path)
    assert r[:etag]

    stored = File.join(@store_dir, "#{BUCKET}/resume_e2e.bin")
    ...
    File.delete(src_path) if src_path && File.exist?(src_path)
    cleanup_suite_tmp("mbc_resume_e2e")
  end

  def test_resume_upload_requires_loadable_constant
    assert S3BaseClient.const_defined?(:ResumeUpload),
           "ResumeUpload must be loadable without the umbrella file"
    resume = S3BaseClient::ResumeUpload.new(
      client: @client, logger: Logger.new(File::NULL)
    )
    assert_respond_to resume, :call
  end
