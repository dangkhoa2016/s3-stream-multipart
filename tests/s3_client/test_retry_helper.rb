...
class S3RetryHelperTest < Minitest::Test
  def setup
    ...
    def @fake_client.transient_errors = [EOFError, Errno::ECONNRESET]
    def @fake_client.log_warn(*); end
    def @fake_client.log_error(*); end
    def @fake_client.backoff_with_jitter(attempt) = 0.01
  ...
  def test_retry_with_backoff_super_path
    host = Class.new(S3BaseClient) do
      ...
      def log_warn(*); end
      def log_error(*); end
      def transient_errors = [EOFError, Errno::ECONNRESET]
      def backoff_with_jitter(attempt) = 0.01
    end.new
