...
module Validator
  ...
  # Validate an S3-compatible endpoint URL.
  #
  # @param endpoint [String] the endpoint URL to validate
  # @raise [ArgumentError] if endpoint is not a valid URL with http/https scheme and non-empty host
  # @return [void]
  def validate_endpoint!(endpoint)
    raise ArgumentError, "endpoint cannot be empty" if endpoint.to_s.strip.empty?
    ...
    return unless uri.host.to_s.strip.empty?

    raise ArgumentError,
          "endpoint has no hostname: #{endpoint}"
  rescue URI::InvalidURIError
    raise ArgumentError, "endpoint is not a valid URL: #{endpoint}"
  end
end
