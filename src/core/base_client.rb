...
require_relative "upload_completion"
...
class S3BaseClient
  ...
  def list_objects(prefix: nil, delimiter: "/", max_keys: 100, bucket: nil, paginate: true)
    all_contents = []
    all_common_prefixes = []
    ...
    loop do
      ...
      result = _ops_execute(:get, "/", bucket: bucket, query: query) do |resp|
        validate_xml_response!(resp, "list_objects")
        xml = resp.body
        log_debug "list_objects: #{xml.bytesize}B XML"
        parse_list_objects_xml(xml)
      end

      all_contents.concat(result[:contents])
      all_common_prefixes.concat(result[:common_prefixes])

      break unless paginate
      break unless result[:is_truncated]
      break if (continuation_token = result[:next_continuation_token]).nil?
    end

    { contents: all_contents, common_prefixes: all_common_prefixes }
  end
  ...
  # Validate that an HTTP response contains XML before parsing.
  # Prevents cryptic REXML parse errors when the server returns HTML or other formats.
  #
  # @param resp    [Net::HTTPResponse] the response to validate
  # @param context [String] operation name for the error message
  # @raise [S3Error] if the response content-type is explicitly non-XML
  def validate_xml_response!(resp, context)
    ct = resp["content-type"].to_s
    return if ct.empty? || ct.include?("xml")

    snippet = resp.body.to_s[0, 80].strip
    raise S3Error.new(
      resp.code.to_s,
      "#{context}: expected XML response, got #{ct.inspect} (#{snippet}...)",
      resp["x-amz-request-id"]
    )
  end
  ...
  def _init_shared(region:, access_key_id:, secret_access_key:, session_token: nil, signature_version: :v4)
    @region          = region
    @session_token   = session_token
    @access_key_id   = access_key_id
    @secret_access_key = secret_access_key
    @signature_version = (signature_version || :v4).to_sym
