...
class MultiBucketUploadTransport
  ...
  def put_single(key, body, headers, bucket:)
    ...
    if body.is_a?(String) && File.exist?(body)
      File.open(body, 'rb') do |file|
        ...
        raise_upload_error!(resp) unless resp.is_a?(Net::HTTPSuccess)
  ...
  def raise_upload_error!(response)
    s3_code, s3_message = parse_s3_error_body(response.body)
    detail = s3_code ? " [#{s3_code}#{": #{s3_message}" if s3_message && s3_message != s3_code}]" : ""
    raise S3BaseClient::UploadError, "Upload failed: #{response.code} #{response.message}#{detail}"
  end

  def parse_s3_error_body(body)
    return [nil, nil] unless body.is_a?(String) && !body.empty?
