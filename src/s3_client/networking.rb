...
class S3Client
  ...
  def http_start(uri, &) = _http_start(uri, &)
  ...
  def _ops_execute(method, key, bucket: nil, query: nil, headers: {}, body: nil, &)
    perform_request(method, key, body: body, headers: headers, query: query, &)
  end
  ...
  def _format_delete_result(_key) = 204

  def _format_abort_result(key, upload_id) = { key: key, upload_id: upload_id, status: "aborted" }
