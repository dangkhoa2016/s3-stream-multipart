...
module S3RetryHelper
  ...
  def self.retry_with_backoff(max_retries:, client:, backoff_base: nil, context: "", on_retry: nil, &block)
    client.retry_with_backoff(max_retries: max_retries, context: context, on_retry: on_retry, &block)
  end
  ...
  #
  # @return [Object] the return value of the block
  def retry_with_backoff(max_retries:, client: nil, backoff_base: nil, context: "", on_retry: nil, &block)
    if client.nil? || client.equal?(self)
      super(max_retries: max_retries, context: context, on_retry: on_retry, &block)
    else
      client.retry_with_backoff(max_retries: max_retries, context: context, on_retry: on_retry, &block)
    end
  end
