...
class S3MultiBucketClient
  ...
  def http_start(bucket, key, &)
    _http_start(build_uri(bucket, key), &)
  end
  ...
  def make_http_request(uri, &) = _http_start(uri, &)
  ...
  def _format_delete_result(key) = { key: key, status: 'deleted' }
