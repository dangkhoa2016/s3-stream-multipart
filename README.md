...
[![CI](https://github.com/dangkhoa2016/s3-upload/actions/workflows/ci.yml/badge.svg)](https://github.com/dangkhoa2016/s3-upload/actions/workflows/ci.yml)
[![Gem Version](https://img.shields.io/gem/v/s3-upload)](https://rubygems.org/gems/s3-upload)
...
Ruby **>= 3.2** (uses `Data.define`, endless methods, and other modern features).

## Installation

```ruby
# Gemfile
gem "s3-upload", github: "dangkhoa2016/s3-upload"
```

Or build locally:

```bash
gem build s3-upload.gemspec
gem install s3-upload-3.0.0.gem
```
...
```bash
# Upload a file
s3-up upload photo.jpg --bucket my-bucket --key photos/photo.jpg

# Download a file
s3-up download photos/photo.jpg /tmp/photo.jpg --bucket my-bucket

# Generate a presigned URL (expires in 1 hour)
s3-up presign photos/photo.jpg --bucket my-bucket --expires 3600

# Delete an object
s3-up delete photos/photo.jpg --bucket my-bucket

# Upload a directory
s3-up upload-dir ./build --bucket my-bucket --prefix site/

# List active multipart uploads
s3-up list-uploads --bucket my-bucket
...
s3-up upload photo.jpg --key photos/photo.jpg

# Custom endpoint (MinIO, R2, etc.)
s3-up --endpoint https://minio.local:9000 upload photo.jpg \
  --bucket my-bucket --key photos/photo.jpg
```

Run `s3-up --help` for all commands and options.
...
- [Client Comparison](docs/COMPARISON.md) — when to use each client
- [Migration Guide v1 → v2](docs/MIGRATION_GUIDE.md) — upgrading from 1.0.0

## Development

```bash
git clone https://github.com/dangkhoa2016/s3-upload.git
cd s3-upload
bundle install
