...
task :'rbs:validate' do
  sh "rbs", "validate", "sig/s3-upload.rbs"
end
...
# ── Release tasks ─────────────────────────────────────────────────────
gemspec = Gem::Specification.load("s3-upload.gemspec")

desc "Build the gem into pkg/"
task :build do
  sh "gem", "build", "s3-upload.gemspec"
  FileUtils.mkdir_p("pkg")
  FileUtils.mv(Dir.glob("s3-upload-*.gem"), "pkg/")
end

desc "Build and install the gem locally"
task install: :build do
  sh "gem", "install", "pkg/s3-upload-#{gemspec.version}.gem"
end
...
task :release do
  ...
  sh "gem", "push", "pkg/s3-upload-#{gemspec.version}.gem"
end
